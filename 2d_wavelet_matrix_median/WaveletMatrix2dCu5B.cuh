...
__global__ void WaveletMatrix2dCu5B_UpSweep_gpu(const SrcT mask, const uint16_t block_pair_num, const XYIdxT size_div_w, const SrcT* __restrict__ src, DstT* __restrict__ dst, BlockT* __restrict__ nbit_bp, const XYIdxT* __restrict__ nsum_zeros_buf, XYIdxT* __restrict__ nsum_zeros_buf2, const uint32_t bv_block_byte_div32, const uint32_t buf_byte_div32, const XIdxT* __restrict__ idx_p, const XIdxT inf, XIdxT* __restrict__ wm, XIdxT* __restrict__ nxt_idx, XYIdxT* __restrict__ wm_nsum_scan_buf, const XYIdxT cwm_buf_byte_div32) {
    ...
    nsum_zeros_buf = (XYIdxT*)((uint8_t*)nsum_zeros_buf + buf_byte_y_offset);
    nsum_zeros_buf2 = (XYIdxT*)((uint8_t*)nsum_zeros_buf2 + buf_byte_y_offset);
    ...
    const XYIdxT nsum_offset = nsum_zeros_buf[blockIdx.x];
    ...
    for (XYIdxT ka = 0; ka < block_pair_num; ka += WARP_SIZE / SRC_CACHE_DIV) {
        ...
        s = __shfl_sync(0xffffffff, s, threadIdx.y, WARP_SIZE);
        s += t + pre_sum;
        ...
        for (XYIdxT j = 0, i = ibb + WARP_SIZE / SRC_CACHE_DIV * threadIdx.y; j < WARP_SIZE / SRC_CACHE_DIV; ++j, ++i) {
            ...
            const WarpWT e_nbit = __shfl_sync(0xffffffff, my_bits, j, WARP_SIZE);
            const XYIdxT e_nsum = __shfl_sync(0xffffffff, s, j, WARP_SIZE);
...
__global__ void WaveletMatrix2dCu5B_ExclusiveSum(XYIdxT* __restrict__ nsum_scan_buf, XYIdxT* __restrict__ nsum_zeros_buf2, BlockT* __restrict__ nsum_p, const uint32_t buf_byte_div32, const uint32_t bv_block_byte_div32) {
    ...
    nsum_zeros_buf2 = (XYIdxT*)((uint8_t*)nsum_zeros_buf2 + (size_t)blockIdx.x * (buf_byte_div32*32ull));
...
__global__ void WaveletMatrix2dCu5B_first_gpu_multi(const SrcT mask, uint16_t block_pair_num, const XYIdxT size_div_warp, SrcT* __restrict__ src, SrcT* __restrict__ dst, XYIdxT* __restrict__ nsum_scan_buf, const uint32_t buf_byte_div32, XIdxT* __restrict__ buf_idx, const int W, const XYIdxT WH, const SrcT* __restrict__ src_const) {
    using WarpWT = uint32_t;
    constexpr int WARP_SIZE = 8 * sizeof(WarpWT);
    static_assert(WARP_SIZE == 32);
    static constexpr int THREAD_PER_GRID = ThreadsDimY * WARP_SIZE;
    ...
    for (XYIdxT ka = 0; ka < block_pair_num; ka += WARP_SIZE, ibb += THREAD_PER_GRID) {
        for (XYIdxT kb = 0, i = ibb + WARP_SIZE * threadIdx.y; kb < WARP_SIZE; ++kb, ++i) {
            ...
            for (int c = 0; c < CH_NUM; ++c) {
                ...
                const int v = src_vbuf[threadIdx.x + c * WARP_SIZE];
            }
            __syncwarp();
            for (int c = 0; c < CH_NUM; ++c) {
...
__device__
inline IdxType WaveletMatrix2dCu5B_median2d_rank0(const IdxType i, const BlockT* __restrict__ nbit_bp) {
    using WordT = decltype(BlockT::nbit);
    constexpr int WORD_SIZE = 8 * sizeof(WordT);
    static_assert(WORD_SIZE == 32 || WORD_SIZE == 64);
...
template<int THREADS_NUM_W, int THREADS_NUM_H, int VAL_BIT_LEN, bool CUT_BORDER, typename XYIdxT, typename XIdxT, typename ValT, typename BlockT, typename ResT, typename ResTableT>
__global__ void WaveletMatrix2dCu5B_median2d_cu(
    const int H, const int W, const int r, ResT* __restrict__ res_cu, const BlockT* __restrict__ wm_nbit_bp, const uint32_t nsum_pos,
    ...
    for (int8_t h = (VAL_BIT_LEN >= 0 ? VAL_BIT_LEN : val_bit_len); h--; ) {
        ...
        for (int8_t j = w_bit_len; j--; ) {
            ...
            const XYIdxT l_ya_xa_rank0 = WaveletMatrix2dCu5B_median2d_rank0(l_ya_xa, wm_nbit_bp);
            const XYIdxT l_ya_xb_rank0 = WaveletMatrix2dCu5B_median2d_rank0(l_ya_xb, wm_nbit_bp);
            const XYIdxT l_yb_xb_rank0 = WaveletMatrix2dCu5B_median2d_rank0(l_yb_xb, wm_nbit_bp);
            const XYIdxT l_yb_xa_rank0 = WaveletMatrix2dCu5B_median2d_rank0(l_yb_xa, wm_nbit_bp);       
        ...
        const XYIdxT top0 = WaveletMatrix2dCu5B_median2d_rank0(ya, bv_nbit_bp);
        const XYIdxT bot0 = WaveletMatrix2dCu5B_median2d_rank0(yb, bv_nbit_bp);
    ...
    if constexpr(is_same_v<ResTableT, std::nullptr_t>) {
        res_cu[(XYIdxT)y * W + x] = res;
    } else {
        res_cu[(XYIdxT)y * W + x] = res_table[res];
    }
}
...
struct WaveletMatrix2dCu5B {
    ...
    static_assert(WORD_SIZE == 32 || WORD_SIZE == 64);
    using WordT = std::conditional_t<WORD_SIZE == 32, uint32_t, uint64_t>;
    ...
    static_assert(is_same_v<XIdxT, typename MultiWaveletMatrixImpl::T_Type>);
...
public:
    ...
    WaveletMatrix2dCu5B(const int rows, const int cols, const bool use_hw_bit_len = false, const bool alloc_res = true) {
    ...
    void reset(const int rows, const int cols, const bool use_hw_bit_len = false, const bool alloc_res = true) {
        ...
        const size_t buf_byte = 
            ...
            + sizeof(XIdxT) * 2 * size
            + sizeof(ValT) * (CH_NUM == 1 ? 1 : 2) * size;  // Do not reorder if CH_NUM == 1
    ...
    void construct(const ValT *src_cu_const = nullptr, const cudaStream_t main_stream = 0) {
        assert(size > 0 && src_cu != nullptr);
        if (val_bit_len == 0) return;
...
#undef CALC_SRCB_SIZE
        ...
        XYIdxT *nsum_zeros_buf = (XYIdxT*)(nxt_cu + size);
        XYIdxT *nsum_zeros_buf2 = nsum_zeros_buf + nsum_scan_buf_len;
        XIdxT *buf_idx = (XIdxT*)(nsum_zeros_buf2 + nsum_scan_buf_len);
        XIdxT *nxt_idx = (XIdxT*)(buf_idx + size);

        int h = val_bit_len - 1;
        if (src_cu_const == nullptr) {
            WaveletMatrix2dCu5B_first_gpu_multi<CH_NUM, ThreadsDimY, 0> << <grid_x, thread, 0, main_stream >> > (ValT(mask / 2),block_pair_num, size_div_warp, src_cu, now_cu, nsum_zeros_buf, buf_byte_div32, buf_idx, W, (XYIdxT)W * H, src_cu_const);
        } else {
            WaveletMatrix2dCu5B_first_gpu_multi<CH_NUM, ThreadsDimY, 1> << <grid_x, thread, 0, main_stream >> > (ValT(mask / 2),block_pair_num, size_div_warp, src_cu, now_cu, nsum_zeros_buf, buf_byte_div32, buf_idx, W, (XYIdxT)W * H, src_cu_const);
        }
        BlockT * nsum_p = get_bv_block_cu(h) + nsum_pos;
        WaveletMatrix2dCu5B_ExclusiveSum<CH_NUM, MAX_BLOCK_X, XYIdxT> << < wm_num, grid_x, 0, main_stream >> > (nsum_zeros_buf, nsum_zeros_buf2, nsum_p, buf_byte_div32, bv_block_byte_div32);
        ...
        if constexpr (sizeof(ValT) >= 4) for (; h > 16; --h) {
            ...
            WaveletMatrix2dCu5B_UpSweep_gpu<CH_NUM, ThreadsDimY, SRC_CACHE_DIV, SRCB_S_32> << <grid, thread, SRCB_S_32, main_stream >> > ((SrcT)mask, block_pair_num, size_div_w, (SrcT*)now_cu, (DstT*)nxt_cu, bv_block_nbit_cu_h, nsum_zeros_buf, nsum_zeros_buf2, bv_block_byte_div32, buf_byte_div32, buf_idx, inf, cwm, nxt_idx, WM_nsum_buf, cwm_buf_byte_div32);

            if (h == 0) break;
            BlockT* nsum_p = get_bv_block_cu(h - 1) + nsum_pos;
            WaveletMatrix2dCu5B_ExclusiveSum<CH_NUM, MAX_BLOCK_X, XYIdxT> << < wm_num, grid_x, 0, main_stream >> > (nsum_zeros_buf2, nsum_zeros_buf, nsum_p, buf_byte_div32, bv_block_byte_div32);
            swap(nsum_zeros_buf, nsum_zeros_buf2);
            swap(now_cu, nxt_cu);
            swap(buf_idx, nxt_idx);
        }
        if constexpr (sizeof(ValT) >= 4) if (h == 16 || (is_same_v<ValT, uint32_t> && h >= 0)) do {
            using SrcT = uint32_t;
            using DstT = uint16_t;
            mask >>= 1;
            BlockT* bv_block_nbit_cu_h = get_bv_block_cu(h);
        ...
        if constexpr (sizeof(ValT) >= 2) for (; h > 8; --h) {
            ...
            XYIdxT* WM_nsum_buf = WM.get_nsum_buf(h * CH_NUM);
            XIdxT* cwm = WM.get_src_p(h * CH_NUM);
            WaveletMatrix2dCu5B_UpSweep_gpu<CH_NUM, ThreadsDimY, SRC_CACHE_DIV, SRCB_S_16> << <grid, thread, SRCB_S_16, main_stream >> > ((SrcT)mask, block_pair_num, size_div_w, (SrcT*)now_cu, (DstT*)nxt_cu, bv_block_nbit_cu_h, nsum_zeros_buf, nsum_zeros_buf2, bv_block_byte_div32, buf_byte_div32, buf_idx, inf, cwm, nxt_idx, WM_nsum_buf, cwm_buf_byte_div32);
        ...
        if constexpr (sizeof(ValT) >= 2) if (h == 8 || (is_same_v<ValT, uint32_t> && h >= 0)) do {
            using SrcT = uint16_t;
            using DstT = uint8_t;
            mask >>= 1;
            BlockT* bv_block_nbit_cu_h = get_bv_block_cu(h);
        ...
        for (; h >= 0; --h) {
            ...
            XYIdxT* WM_nsum_buf = WM.get_nsum_buf(h * CH_NUM);
            XIdxT* cwm = WM.get_src_p(h * CH_NUM);
            WaveletMatrix2dCu5B_UpSweep_gpu<CH_NUM, ThreadsDimY, SRC_CACHE_DIV, SRCB_S_8> << <grid, thread, SRCB_S_8, main_stream >> > ((SrcT)mask, block_pair_num, size_div_w, (SrcT*)now_cu, (DstT*)nxt_cu, bv_block_nbit_cu_h, nsum_zeros_buf, nsum_zeros_buf2, bv_block_byte_div32, buf_byte_div32, buf_idx, inf, cwm, nxt_idx, WM_nsum_buf, cwm_buf_byte_div32);
    ...
    void median2d(const int r, const ResTableT* res_table = nullptr) {
        ...
        if (is_same_v<ResTableT, std::nullptr_t> == false && res_table == nullptr) {printf("Median2d Error: res_table is null."); return;}
        static_assert(is_same_v<ResTableT, std::nullptr_t> || (sizeof(ResTableT) <= sizeof(ValT)));

        static_assert(TH_W * TH_H <= 1024);


        constexpr int THREADS_NUM_W = TH_W;
        const dim3 thread(THREADS_NUM_W, TH_H);
        const dim3 grid(div_ceil<size_t>((CUT_BORDER ? W - 2 * r: W), THREADS_NUM_W), div_ceil<size_t>((CUT_BORDER ? H - 2 * r : H), TH_H));
        ...
        const BlockT* wm_bv_block_nbit_cu_first = WM.get_bv_block_cu(w_bit_len - 1, val_bit_len - 1); // 
        ...
        using ResT = std::conditional_t<is_same_v<ResTableT, std::nullptr_t>, ValT, ResTableT>;

        const int Wc = (CUT_BORDER ? W - 2 * r : W);
        const int Hc = (CUT_BORDER ? H - 2 * r : H);

        constexpr int VAL_BIT_LEN = (sizeof(ValT) < 4) ? MAX_BIT_LEN : -1;
        WaveletMatrix2dCu5B_median2d_cu<THREADS_NUM_W, TH_H, VAL_BIT_LEN, CUT_BORDER, XYIdxT, XIdxT, ValT> << <grid, thread >> >
            (Hc, Wc, r, (ResT*)res_cu, wm_bv_block_nbit_cu_first, nsum_pos, WM.get_bv_block_h_byte_div32(), bv_block_len,
    ...
    vector<vector<ResTableT>> get_res() {
        ...
        auto res = vector(H, vector<ResTableT>(W));
